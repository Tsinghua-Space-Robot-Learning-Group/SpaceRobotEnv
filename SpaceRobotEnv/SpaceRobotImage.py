...
MODEL_XML_PATH = os.path.join('mujoco_files', 'spacerobot', 'spacerobot_image.xml')
DEFAULT_SIZE = 500
...
class RobotEnv(gym.GoalEnv):
    ...
    def __init__(self, model_path, initial_qpos, n_substeps):
        ...
        self.observation_space = spaces.Dict(dict(
            ...
            base =spaces.Box(-np.inf, np.inf, shape=obs['base'].shape, dtype='float32'),
            image=spaces.Box(-np.inf, np.inf, shape=obs['image'].shape, dtype='float32'),
            rawimage=spaces.Box(-np.inf, np.inf, shape=obs['rawimage'].shape, dtype='float32'),
            depth=spaces.Box(-np.inf, np.inf, shape=obs['depth'].shape, dtype='float32'),
        ))
...
class SpacerobotEnv(RobotEnv):
    ...
    def _sample_goal(self):
        ...
        # #固定点抓取 改变位置
        # goal[0] = self.initial_gripper_xpos[0] - 0.2 #self.np_random.uniform(-0.45, 0) # 目标为移动到随机位置
        # goal[1] = self.initial_gripper_xpos[1] -0.37  #self.np_random.uniform(-0.3, 0.3)
        # goal[2] = self.initial_gripper_xpos[2] + 0.22#self.np_random.uniform(0.1, 0.3)

        #固定点抓取 改变位置
        goal[0] = self.initial_gripper_xpos[0] - 0.1 #self.np_random.uniform(-0.45, 0) # 目标为移动到随机位置
        goal[1] = self.initial_gripper_xpos[1] + 0  #self.np_random.uniform(-0.3, 0.3)
        goal[2] = self.initial_gripper_xpos[2] + 0.3#self.np_random.uniform(0.1, 0.3)


        # # 加入一定的噪声来使得训练的策略更鲁棒
        #
        # goal[0] = self.initial_gripper_xpos[0] + self.np_random.uniform(-0.05, -0.15) #self.np_random.uniform(-0.45, 0) # 目标为移动到随机位置
        # goal[1] = self.initial_gripper_xpos[1] + self.np_random.uniform(-0.05, -0.05)  #self.np_random.uniform(-0.3, 0.3)
        # goal[2] = self.initial_gripper_xpos[2] + self.np_random.uniform(0.25, 0.35)
